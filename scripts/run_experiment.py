...
from cibica.data import data_dir
...
def run_experiments_with_real_data(n_triplets=500):
    ...
    for i, filename in enumerate(filenames):
        ...
        BS_crop = cv2.imread(os.path.join(str(_DATA), 'black_sphere_ROI', filename + '.png'))
        G_crop  = cv2.imread(os.path.join(str(_DATA), 'green_back_ROI',   filename + '.png'))
        if BS_crop is None:
